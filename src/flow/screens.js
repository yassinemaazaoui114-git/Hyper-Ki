...
export function stepTitle(){
  ...
  if(pressed.start){
    ...
    else if(game.titleSel===3){game.state='records';}
...
export function stepMSettings(){
  ...
  if(pressed.start){
    ...
    else{game.diff=game.selDiff;if(game.mode==='2p')recordPick(game.p1i);startMatch();}
  }
...
export function stepRecords(){
  if(keys.down&&pressed.punch){resetProfile();SFX.confirm();return;}
  if(pressed.back||pressed.start){SFX.select();game.state='title';}
}
